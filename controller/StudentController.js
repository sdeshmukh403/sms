...
exports.getStudentList = (req, res) =>{
  ...
  User.findAll({where: filterWhere,attributes:['id', 'firstname','lastname', 'roll_no', 'gender', 'section_id','address', 'dob', 'phone', 'email', 'class'] , 
  raw:true, order: [
    ['id', 'DESC']
...
exports.getStudentDetail = (req, res) =>{ 
    User.findAll({where:{id:req.params.id}, attributes: ['id', 'firstname', 'lastname','roll_no', 'gender', 'section_id','address', 'dob', 'phone', 'email', 'class'], raw:true}).then(function (result) { 
 ...
 User.create({
   ...
   exports.postEditStudent = (req, res) => {
    User.findAll({where:{id:req.query.id}    
      ...
      return res.render('edit-student', {title: 'Student',
       main_heading:'Update Students', sub_heading:'Update student data', data: result });
    }).catch(err => {
       return res.json({ msg: "Something went wrong",  success:false });
   });    
   }
